...
**Constraints enforced in generator (not only in Groq LLM prompt):**
...
**Stretch:** `GroqEmailGenerator` (using Groq API/SDK) implementing the same `generate_email` interface, with a **post-generation validator** (word count, banned phrases, no fake referral language).
...
| Groq LLM rewriting | `GroqEmailGenerator` + `EmailQualityValidator` |
